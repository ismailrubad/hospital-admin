...
const AllImages = ({ closeModal, getImageData, selectOneMood, prevSelectedImageIds }) => {
  ...
  useEffect(() => {
    axios.get(`http://3.6.216.223/admin/api/image`)
  ...
  const deleteImageHandler = id => {
    axios.get(`http://3.6.216.223/admin/api/image/remove?id=${id}`)
  ...
  const uploadImageHandler = (uploadImageInfo) => {
    axios.post(`http://3.6.216.223/admin/api/image/add/`, uploadImageInfo)
      .then(res => {
